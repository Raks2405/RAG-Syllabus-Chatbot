...
export class StudentView extends AbstractView {
  ...
  async extractPdfText(url) {
    ...
    let pdf;
    try {
      // *** CRITICAL CHANGE FOR DEPLOYMENT ISSUE (Failed to fetch) ***
      // To fix the "Failed to fetch" error often caused by Web Worker security restrictions 
      // (a common issue when using pdf.js on deployment), we set disableWorker: true.
      // This loads the PDF on the main thread, which can be slower but avoids the cross-origin worker error.
      console.log("📥 Attempting to fetch PDF document and disabling Web Worker...");
      pdf = await pdfjsLib.getDocument({ url: url, disableWorker: true }).promise;
    } catch (error) {
        console.error("❌ PDF document loading failed (check CORS/CSP):", error);
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      // Join the string contents of all text items on the page
      text += content.items.map(item => item.str).join(" ") + " ";
    }
    
    // Clean up extra whitespace before returning
    return text.trim();
  }
