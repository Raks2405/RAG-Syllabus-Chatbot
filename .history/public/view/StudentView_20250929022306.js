...
export class StudentView extends AbstractView {
  ...
  async talkToLLM(prompt) {
    // --- Direct Gemini call from browser (no proxy) ---
    const API_KEY = "AIzaSyD7EuJGy7L-Y9mbT3UgGvGq4OAYi5dpiBE";               // <- put your key here (restrict it to your domains!)
    const MODEL   = "gemini-2.5-flash";
    const API_URL = `https://generativelanguage.googleapis.com/v1/models/${encodeURIComponent(MODEL)}:generateContent?key=${API_KEY}`;
    ...
    // extractor that handles 2.5 responses (text, tool calls, code, etc.)
    ...
    try {
      ...
      if (!response.ok) {
        console.error("Gemini error:", textBody);
      ...
      // Your UI expects a string reply
      return extractTextFromGemini(data);
    } catch (error) {
      console.error('Error occurred while calling Gemini:', error);
      throw error;
    }
  }
}
