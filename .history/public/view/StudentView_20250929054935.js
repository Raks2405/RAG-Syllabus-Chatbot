...
// NOTE: This URL must match the Firebase rewrite rule defined in firebase.json
const PDF_EXTRACTION_API_URL = '/api/extract-syllabus-text';
...
        } else {
          try {
            // === START SERVER-SIDE EXTRACTION LOGIC (The new reliable method) ===
            console.log(`📡 Sending request to backend to fetch and extract PDF content for ${syllabus.courseNumber}...`);
            
            const response = await fetch(PDF_EXTRACTION_API_URL, { 
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ pdfUrl: pdfUrl }) // Send the URL to the server
            });
            
            if (!response.ok) {
                // Improved error handling to catch HTML response (the root cause of the SyntaxError)
                const responseText = await response.text();
                if (responseText.startsWith('<!doctype')) {
                    throw new Error(`API Endpoint not found. Check firebase.json rewrite rule for ${PDF_EXTRACTION_API_URL}.`);
                }
                ...
                try {
                    ...
                    errorData = { error: `Server returned non-JSON error status ${response.status}: ${responseText.substring(0, 50)}...` };
                }
                throw new Error(errorData.error || `HTTP error! status: ${response.status}`);
            }

            const data = await response.json();
            const syllabusText = data.text; // Text comes back clean from the server

            console.log(`📑 Extracted text from PDF (${syllabus.courseNumber}) received from server.`);
            this.syllabusTextMap[index] = syllabusText;
            // === END SERVER-SIDE EXTRACTION LOGIC ===
...
    }
    ...
    // === DELETED: The problematic client-side extractPdfText function is removed. ===
