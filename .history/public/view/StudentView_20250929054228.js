...
// NOTE: Set the base URL for your Firebase Cloud Function or API Gateway here.
...
        } else {
          try {
            // === SERVER-SIDE EXTRACTION LOGIC (This is where the error occurs) ===
...
            
            if (!response.ok) {
                // Try to parse error as JSON, but if it fails, it means we got HTML
                const responseText = await response.text();
                try {
                    const errorData = JSON.parse(responseText);
                    throw new Error(errorData.error || `HTTP error! status: ${response.status}`);
                } catch (jsonError) {
                    // THIS IS THE LIKELY PATH NOW
                    throw new Error(`API Endpoint not found or server error. Received status ${response.status} and non-JSON content.`);
                }
            }

            const data = await response.json(); // <-- THIS LINE CAUSES THE SyntaxError
