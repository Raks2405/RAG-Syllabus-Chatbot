...
async extractPdfText(url) {
  ...
  // 1) Fetch the file so we can see status + content-type explicitly
  const res = await fetch(url, { method: "GET" });

  console.log(
    "PDF fetch status:", res.status,
    "| ok:", res.ok,
  ...
  if (!ct.includes("pdf")) {
    ...
    console.error("URL is not a PDF. content-type:", ct, "sample:", sample);
    throw new Error(
      "URL did not return a PDF. You must store a Firebase Storage *download* URL " +
      "(looks like .../v0/b/<bucket>/o/<path>.pdf?alt=media&token=<...>)."
    );
  }

  // 2) Hand the bytes to pdf.js (avoids some cross-origin issues)
  ...
  async talkToLLM(prompt) {
    const LLAMA_API_URL = " https://gemini-proxy-113410734669.us-central1.run.app/generate"; // Replace with your actual endpoint
