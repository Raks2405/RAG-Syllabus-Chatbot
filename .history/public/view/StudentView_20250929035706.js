...
export class StudentView extends AbstractView {
  ...
  // Keep your pdfjsLib.workerSrc line above this if you have it
...
async extractPdfText(url) {
  ...
  // 1) Do the fetch yourself so you can see real errors and bypass pdf.js CORS issues
  let resp;
  try {
    resp = await fetch(url, { method: "GET", mode: "cors" });
  } catch (e) {
    console.error("❌ PDF fetch failed before response:", e);
  ...
  if (!resp.ok) {
    ...
    console.error("❌ PDF fetch HTTP error", resp.status, ct, preview.slice(0, 300));
    throw new Error(`Failed to fetch PDF: ${resp.status}`);
  }

  // Optional sanity: must look like a PDF
  const ct = resp.headers.get("content-type") || "";
  if (!ct.includes("pdf")) {
    console.warn("⚠️ Content-Type is not application/pdf:", ct);
    // not fatal—some CDNs omit it; continue
  }

  const pdfBytes = await resp.arrayBuffer();

  // 2) Hand bytes to pdf.js (no cross-origin read from inside pdf.js)
