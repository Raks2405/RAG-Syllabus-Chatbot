...
async function extractPdfText(url) {
  console.log("📄 extractPdfText:", url);
  if (!url || typeof url !== "string") throw new Error("Invalid PDF URL");

  // Fetch the bytes yourself (clear errors + bypasses pdf.js cross-origin fetch)
  let resp;
  try {
  ...
  if (!resp.ok) {
    ...
    console.error("❌ HTTP error:", resp.status, ct, peek.slice(0, 200));
