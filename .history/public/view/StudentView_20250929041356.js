...
export class StudentView extends AbstractView {
  ...
  // keep your <script src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.10.377/pdf.min.js"></script>
// version in index.html. Make sure the workerSrc matches that version.

async extractPdfText(url) {
  // Set the pdf.js workerSrc before using pdfjsLib
  ...
  // 1) Fetch the PDF yourself (clear errors if it fails)
  let resp;
  try {
    resp = await fetch(url, { method: "GET", mode: "cors", credentials: "omit" });
  } catch (e) {
    console.error("❌ PDF fetch failed (network/CORS):", e);
    throw new Error("Failed to fetch PDF (network/CORS)");
  }

  if (!resp.ok) {
    const ct = resp.headers.get("content-type");
    const peek = await resp.text().catch(() => "");
    console.error("❌ PDF fetch HTTP error:", resp.status, ct, peek.slice(0, 300));
    throw new Error(`Failed to fetch PDF: ${resp.status}`);
  }

  const ct = (resp.headers.get("content-type") || "").toLowerCase();
  if (!ct.includes("pdf")) {
    console.warn("⚠️ Response content-type is not application/pdf:", ct);
    // some CDNs omit the header; continue anyway
  }

  const bytes = await resp.arrayBuffer();

  // 2) Hand bytes to pdf.js (no internal cross-origin fetch)
