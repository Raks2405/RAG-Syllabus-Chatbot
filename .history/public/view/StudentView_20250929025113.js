...
export class StudentView extends AbstractView {
  ...
  try {
    // 1) Try a plain GET so we can see status codes clearly
    ...
    // Optional sanity: warn if it doesn't look like a PDF
    const ct = res.headers.get("content-type") || "";
    if (!ct.includes("pdf")) {
      console.warn("⚠️ Unexpected content-type for PDF:", ct);
    }

    // 2) Feed bytes to pdf.js (avoids some cross-origin quirks)
    ...
    console.error("❌ PDF fetch/extract error:", err);
    throw err;
  }
}
